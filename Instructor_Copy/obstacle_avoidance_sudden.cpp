// Reduced Maximum speed and increased obstacle avoidance threshold
...
void loop() {
  ...
  if (distance_front <= obstacleThresholdMin) {
    stopCar();
    delay(time);

    findAndRotateToFreePath();
  } else {
    moveForward(travelDuration);
  }

  // I don't think it is needed here because moveforward already delays for some tome
  // delay(500); // Delay needed for ultrasonic sensor to read next value
}
